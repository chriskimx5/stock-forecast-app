...
type PredictResp = {
  ...
  pred_close: number;
  pred_close_1sigma_low: number;
  pred_close_1sigma_high: number;
  model_window: number;
  trained_at?: string;
  rmse_return?: number;
  cached: boolean;
...
export default function Page() {
  ...
  async function loadPrices() {
    ...
    const data = await callJson(`${API_BASE}/prices/${t}?limit=400`);
    setPrices(data);
    setPred(null);
  }
  ...
  async function ingest() {
    const t = ticker.trim().toUpperCase();
    await callJson(`${API_BASE}/ingest/${t}?period=1y&interval=1d`, { method: "POST" });
    await loadPrices();
  }

  async function trainModel() {
    const t = ticker.trim().toUpperCase();
    const data = await callJson(`${API_BASE}/train/${t}?window=20`, { method: "POST" });
    setTrain(data);
  }

  async function predict() {
    const t = ticker.trim().toUpperCase();
    const data = await callJson(`${API_BASE}/predict/${t}?ttl=60`);
    setPred(data);
  }
  ...
  return (
    <main style={{ padding: 24, fontFamily: "system-ui", maxWidth: 980, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap", marginTop: 16 }}>
        ...
        <button onClick={loadPrices} style={btn}>
          Load prices
        </button>
        <button onClick={ingest} style={btn}>
          Ingest (1y)
        </button>
        <button onClick={trainModel} style={btn}>
          Train
        </button>
        <button onClick={predict} style={btn}>
          Predict
        </button>

        <span style={{ marginLeft: "auto", color: status.startsWith("OK") ? "green" : "#555" }}>
          {status}
        </span>
      </div>

      <section style={{ marginTop: 18, padding: 16, border: "1px solid #eee", borderRadius: 12 }}>
        <h2 style={{ marginTop: 0, fontSize: 18 }}>Chart</h2>
        {chartData.length === 0 ? (
        ...
        <div style={card}>
          ...
          {pred ? (
            <div style={mono}>
              ...
              <div>
                1σ band: [{fmt(pred.pred_close_1sigma_low)}, {fmt(pred.pred_close_1sigma_high)}]
              </div>
      ...
      <p style={{ marginTop: 18, color: "#666" }}>
        Tip: use “Ingest” for new tickers to populate the DB before charting/training.
      </p>
    </main>
  );
}
